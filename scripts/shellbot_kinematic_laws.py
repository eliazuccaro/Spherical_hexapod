...
import numpy as np
from iDynTree import vectorize

# robot parameters (M indicates parameters modified from first robot design)
...
d1 = 153.5
h1_sup = 63.5
radius = 165.                   # mm - closed robot radius
...
# offset motors variables - WARNING: must be coherent with URDF values
theta_1_offset = 0.
theta_2_offset = 0.
theta_3_offset = -3.5 * pi / 180
theta_4_offset = 88 * pi / 180
theta_1u_offset = 0.67
theta_2u_offset = 1.16

# bottom legs direct kinematics
def f_bottom(leg_num, q1, q2, q3, q4):
...
def f_center(leg_num, q1, q2, q3, q4):
    ...
    # to be set to 0 mm for robot closure
    ...
    z_feedback = l_o0_z+l_12_z*-yinf*cos(q3+q4)-l_23_x*sin(q3)-xinf*sin(q3+q4)-r1*sin(alpha+q3+q4)
    phi_feedback = -pi/3+leg_num*pi/3+q1+q2             # to be set to -72 degrees for robot closure
    theta_feedback = q3 + q4 + alpha + beta + pi/2      # to be set to 360 degrees for robot closure
    psi_feedback = 0
...
def up_inversion(quote, angle):
    ...
    theta_1 = asin((quote - ysup * cos(sum) - xsup * sin(sum) - r2 * sin(delta + sum) - h1_sup) / ls1)
    theta_2 = sum - theta_1

    return np.array([theta_1, theta_2])

# matrix flattening tool
